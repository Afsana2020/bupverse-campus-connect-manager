...
BUPVerse is a Django-based web application that combines multiple student-focused services, including Tutor Finder, Roommate Finder, and Event Management, into a single platform. It integrates real-time recommendation systems and scheduling algorithms powered by core data structures and advanced DSA algorithms, alongside a deep learning NLP architecture (all-MiniLM-L6-v2 Bi-Encoder) for semantic preference matching. The goal is to simplify campus life for students at the Bangladesh University of Professionals (BUP).
